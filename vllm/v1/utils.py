...
class IterationDetails:
    ...
    # Dual-model split of the context/prefill bucket. When embed_model_id
    # is not provided to compute_iteration_details, embed_* fields stay
    # at 0 and decode_ctx_* mirrors num_ctx_*.
    ...
    def __repr__(self) -> str:
        return (
            ...
            f" num_encoder_inputs={self.num_encoder_inputs},"
            f" num_encoder_output_tokens={self.num_encoder_output_tokens},"
...
def compute_iteration_details(
    ...
    as a context request if its output tokens are still 0; an extended
    chunk of chunked prefill falls into this category.

    When ``req_id_to_model_id`` and ``embed_model_id`` are provided
    (dual-model runs), the context bucket is further split into
    decode-prefill (model_id != embed_model_id) and embed-prefill
    (model_id == embed_model_id).

    For single-model runs (no dual_cfg), ``single_runner_kind`` selects
    which bucket to mirror context into:
      - "pooling" / "embed" -> bucket as embed_ctx_* (embed-only baseline)
      - any other value / None -> bucket as decode_ctx_* (legacy default)

    Args:
        scheduler_output: scheduler output for the current iteration.
        req_id_to_model_id: optional dict mapping request_id -> model_id;
            DualModelRunner maintains this and feeds it in. Pass None
            for single-model runs.
        embed_model_id: optional model_id of the embed runner in dual-
            model mode. Pass None for single-model runs.
        single_runner_kind: optional hint for the single-model case.
            Pass "pooling" / "embed" for an embed-only run so prefill
            activity is reported as ectx_* instead of dctx_*. Default
            None -> treat single-model context as decode-prefill.
    ...
    # persistent req_id_to_model_id dict is only populated by the runner
    # during execute_model, which runs AFTER this helper. Without
    # consulting the new-reqs list here, freshly-admitted embed prefills
    # get bucketed as decode prefill because they're not yet in the
    # persistent dict.
    ...
    if not dual_mode:
        # Single-model mode: mirror the context bucket into the kind
        # that matches the runner. Default is decode_ctx_* (legacy).
        # For embed-only runs (e.g. baseline R2), pass
        # single_runner_kind="pooling" so the NVTX label reads ectx_*
        # -- otherwise embed prefills appear under dctx_, which is
        # misleading.
    ...
    return IterationDetails(
        ...
        num_encoder_output_tokens=num_encoder_output_tokens,
