...
// ---------------------------------------------------------------------------
// HB S2b: bounded (grid-stride) COPIES of the kernels above. Originals are
// untouched (bit-identical binary/launch with the budget off); these run
// only when HB_VLLM_ELTWISE_CTA_BUDGET caps the grid below num_tokens.
// Bodies identical apart from the row loop, per-row variance reset, and the
// trailing __syncthreads() protecting the reused shared storage.
template <typename scalar_t, int VEC_SIZE, int NUM_DIMS, bool HasWeight>
__global__ void rms_norm_kernel_hb_bounded(
    ...
    const int64_t weight_stride, const float epsilon, const int num_tokens,
...
template <typename scalar_t, int width, bool HasWeight>
__global__ std::enable_if_t<(width > 0) && _typeConvert<scalar_t>::exists>
fused_add_rms_norm_kernel_hb_bounded(
    scalar_t* __restrict__ input, const int64_t input_stride,
...
void rms_norm(torch::stable::Tensor& out,    // [..., hidden_size]
  ...
  // HB S2b: budget applies to the 2D hidden-norm path only (3D/4D q/k
  // head-norm launches exempt in this first cut).
  const int64_t hb_grid =
      (num_dims == 2) ? vllm::hb_bounded_grid(num_tokens) : num_tokens;
          ...
          VLLM_STABLE_DISPATCH_VEC_SIZE(calculated_vec_size, [&] {
            if (has_weight) {
              if (hb_grid < num_tokens) {
                vllm::rms_norm_kernel_hb_bounded<scalar_t, vec_size,
                                                 tensor_rank, true>
...
#define LAUNCH_FUSED_ADD_RMS_NORM(width, has_weight)                       \
  VLLM_STABLE_DISPATCH_FLOATING_TYPES(                                     \
      input.scalar_type(), "fused_add_rms_norm_kernel", [&] {              \
        if (has_weight) {                                                  \
          if (hb_grid < num_tokens) {                                          \
            vllm::fused_add_rms_norm_kernel_hb_bounded<scalar_t, width, true>  \
                <<<dim3(hb_grid), block, 0, stream>>>(                         \
          ...
          if (hb_grid < num_tokens) {                                          \
            vllm::fused_add_rms_norm_kernel_hb_bounded<scalar_t, width, false> \
                <<<dim3(hb_grid), block, 0, stream>>>(                         \
                    input.mutable_data_ptr<scalar_t>(), input_stride,          \
                    residual.mutable_data_ptr<scalar_t>(),                     \
                    nullptr, epsilon, num_tokens,                              \
                    hidden_size);                                              \
          } else {                                                             \
